...
class Emitter {
	...
	public function on( $event, $callable ) {
		$listener = \MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Carbon_Fields::resolve( 'event_persistent_listener' );
	...
	public function once( $event, $callable ) {
		$listener = \MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Carbon_Fields::resolve( 'event_single_event_listener' );
		$listener->set_callable( $callable );
		return $this->add_listener( $event, $listener );
	}
}