...
namespace MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Loader;

use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Container\Repository as ContainerRepository;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Exception\Incorrect_Syntax_Exception;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Helper\Helper;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Pimple\Container as PimpleContainer;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Service\Legacy_Storage_Service_v_1_5;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Service\Meta_Query_Service;
use MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Service\REST_API_Service;
...
class Loader {
	...
	public function boot() {
		...
		include_once( \MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\DIR . '/core/functions.php' );
	...
	public function load_textdomain() {
		$dir = \MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\DIR . '/languages/';
	...
	protected function enqueue_style( $src, $deps = array() ) {
		...
		wp_enqueue_style(
			'carbon-fields-' . $src,
			\MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.css',
			$deps,
			\MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\VERSION
		);
	}
	...
	protected function enqueue_script( $src, $deps = array() ) {
		...
		wp_enqueue_script(
			'carbon-fields-' . $src,
			\MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.js',
			$deps,
			\MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\VERSION
		);
	}
	...
	public function enqueue_assets() {
		...
		$revisions = \MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Carbon_Fields::service( 'revisions' );
	...
	public function get_json_data() {
		...
		foreach ( $containers as $container ) {
			...
			if ( is_a($container, '\\MZoo\\MBO_Sandbox\\Dependencies\\Carbon_Fields\\Container\\Block_Container', true ) ) {
	...
	public function fetch_association_options() {
		...
		/** @var \MZoo\MBO_Sandbox\Dependencies\Carbon_Fields\Field\Association_Field $field */
